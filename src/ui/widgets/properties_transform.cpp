#include "properties_transform.h"
...
properties_transform::properties_transform(const scenegraph::node_ref& node, QWidget* parent) : properties_standard_form(parent), node(node) {
...
void properties_transform::set_position(glm::vec3 position) {
    if (node.exists()) node->get_transform().set_position(position);
}

void properties_transform::set_rotation(glm::vec3 rotation) {
    if (node.exists()) node->get_transform().set_euler_rotation(glm::radians(rotation));
}

void properties_transform::set_scale(glm::vec3 scale) {
    if (node.exists()) node->get_transform().set_scale(scale);
}

void properties_transform::load_values() {
    if (node.exists()) {
