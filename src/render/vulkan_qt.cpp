...
vk::SurfaceKHR element::vulkan_create_surface_from_qt(QWindow* window) {
    ...
    if (qpa == "xcb") {
        ...
        if (!func) {
            ...
            VkResult r = func(vulkan::get_instance(), &create_info, nullptr, &c_surface);
            if (r != VK_SUCCESS) vk::throwResultException((vk::Result) r, "::vkCreateXlibSurfaceKHR");
        }
    } else if (qpa == "wayland" || qpa == "wayland-egl") {
        ...
        if (!func) {
            ...
            VkResult r = func(vulkan::get_instance(), &create_info, nullptr, &c_surface);
            if (r != VK_SUCCESS) vk::throwResultException((vk::Result) r, "::vkCreateWaylandSurfaceKHR");
        }
    }
#endif
